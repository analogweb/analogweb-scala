...
class ScalaInvocation(path: RequestPathMetadata, val route: Route, val rc: RequestContext, val rsc: ResponseContext, val tc: TypeMapperContext, val rvr: RequestValueResolvers, im: InvocationMetadata) extends Invocation with InvocationArguments {
