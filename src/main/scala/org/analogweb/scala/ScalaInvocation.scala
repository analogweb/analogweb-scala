...
class ScalaInvocation(path: RequestPathMetadata,
                      val route: Route,
                      val rc: RequestContext,
                      val rsc: ResponseContext,
                      val tc: TypeMapperContext,
                      val rvr: RequestValueResolvers,
                      im: InvocationMetadata)
    extends Invocation
    with InvocationArguments {

  def invoke: Object =
    route
      .invoke(new Request(rc, rvr, im, tc))
      .asInstanceOf[Object]
