...
trait Responses {
  ...
  def Created(responseBody: Renderable): HttpStatus = Created.`with`(responseBody)
  def Created: HttpStatus = HttpStatus.CREATED
  def Accepted(responseBody: Renderable): HttpStatus = Accepted.`with`(responseBody)
  def Accepted: HttpStatus = HttpStatus.ACCEPTED
  def NoContent: HttpStatus = HttpStatus.NO_CONTENT
  def MovedPermanently: HttpStatus = HttpStatus.MOVED_PERMANENTLY
  def Found: HttpStatus = HttpStatus.FOUND
  def SeeOther: HttpStatus = HttpStatus.SEE_OTHER
  def NotModified: HttpStatus = HttpStatus.NOT_MODIFIED
  ...
  def MethodNotAllowed(obj: Renderable): HttpStatus = MethodNotAllowed.`with`(obj)
  def MethodNotAllowed: HttpStatus = HttpStatus.METHOD_NOT_ALLOWED
  def NotAcceptable(obj: Renderable): HttpStatus = NotAcceptable.`with`(obj)
  def NotAcceptable: HttpStatus = HttpStatus.NOT_ACCEPTABLE
  def Conflict(obj: Renderable): HttpStatus = Conflict.`with`(obj)
  def Conflict: HttpStatus = HttpStatus.CONFLICT
  def PreconditionFailed(obj: Renderable): HttpStatus = PreconditionFailed.`with`(obj)
  def PreconditionFailed: HttpStatus = HttpStatus.PRECONDITION_FAILED
  def UnsupportedMediaType(obj: Renderable): HttpStatus = UnsupportedMediaType.`with`(obj)
  def UnsupportedMediaType: HttpStatus = HttpStatus.UNSUPPORTED_MEDIA_TYPE
  ...
  def BadGateway(obj: Renderable): HttpStatus = BadGateway.`with`(obj)
  def BadGateway: HttpStatus = HttpStatus.BAD_GATEWAY
  def ServiceUnavailable(obj: Renderable): HttpStatus = ServiceUnavailable.`with`(obj)
  def ServiceUnavailable: HttpStatus = HttpStatus.SERVICE_UNAVAILABLE
}
