...
class ScalaInvocationMetadataFactory(val routeDef: Option[Routes])
  ...
  override def createInvocationMetadatas(
      properties: ApplicationProperties,
      instances: ContainerAdaptor): Collection[InvocationMetadata] = {
    routeDef
      .map { routes =>
        routes.routes.map { d =>
          val metadata: InvocationMetadata =
            new DefaultScalaInvocationMetadata(d.getClass(),
                                               s"${d.method}(${d.rawPath})",
                                               Array.empty,
                                               d)
          metadata
        }.asJava
  ...
  private[this] def createInvocationMetadatasReflectively(
      properties: ApplicationProperties,
      instances: ContainerAdaptor): Collection[InvocationMetadata] = {
    collectClasses(properties)
      .flatMap {
        case c if (containsInvocationClass(c)) => {
          obtainInstance(c, instances)
            .map { instance =>
              instance.routes
                .map { route =>
                  new DefaultScalaInvocationMetadata(c,
                                                     s"${route.method}(${route.rawPath})",
                                                     Array.empty,
                                                     route)
                }
            }
            .getOrElse(Seq
              .empty[InvocationMetadata])
        }
        case _ =>
          Seq.empty
      }
  ...
  protected lazy val classCollectors =
    Seq(new JarClassCollector(), new FileClassCollector())

  protected def classLoader =
  ...
  private[this] def collectClasses(properties: ApplicationProperties) = {
    for {
      packageName <- properties
        ...
        .asScala ++ Seq(Application.DEFAULT_PACKAGE_NAME)
      uri <- ResourceUtils
        .findPackageResources(packageName, classLoader)
        .asScala
  ...
  private def obtainInstance(c: Class[_], instances: ContainerAdaptor): Option[Routes] = {
    Option(
      instances
        .getInstanceOfType(c))
      .map(_.asInstanceOf[Routes])
      .orElse(instantiate(c).toOption)
  }
