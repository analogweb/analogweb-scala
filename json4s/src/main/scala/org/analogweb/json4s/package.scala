...
package object json4s {
  ...
  implicit def asJson4sResolverSyntax[T <: RequestValueResolver](
    ...
    DefaultResolverSyntax(typeOfResolver, request, org.analogweb.json4s.Json4sResolverContext(formats))

  // Serializing JSON responses.
  def asJson(source: AnyRef)(implicit formats: Formats = Serialization.formats(NoTypeHints)) = new ScalaJsonObject((source, formats))
