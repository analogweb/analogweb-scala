...
class ScalaResponseHandlerSpec extends Specification with Mockito {
  ...
  "ScalaResponseHandler" should {
    "resolve simple result" in new Fixture {
      val txt = Text
        .`with`("hello!")
      formatter
        .resolve(txt, metadata, request, response) returns txt
      val actual = handler
        .handleResult(txt, metadata, formatter, request, response, exph, finder)
      actual
    ...
    "resolve Future successfully" in new Fixture {
      ...
      formatter
        .resolve(future, metadata, request, response) returns future
      formatter
        .resolve(txt, metadata, request, response) returns txt
      val actual =
        handler.handleResult(future, metadata, formatter, request, response, exph, finder)
      actual
        .commit(request, response)
      //XXX improve test stability.
      Thread sleep 1000
      actual
        .isInstanceOf[DefaultResponse] === true
    ...
    "resolve Future failure" in new Fixture {
      ...
      val txt = Text
        .`with`("oops!")
      exph
        .handleException(exp) returns txt
      formatter
        .resolve(txt, metadata, request, response) returns txt
      val actual =
        handler.handleResult(future, metadata, formatter, request, response, exph, finder)
  ...
  trait Fixture extends Before {
    ...
    val formatter =
      mock[RenderableResolver]
    val request =
      mock[RequestContext]
    val exph =
      mock[ExceptionHandler]
    val finder =
      mock[ResponseFormatterFinder]
    val response =
      new StubResponseContext
  }
