...
trait ServerApplications {
  ...
  def server(
    ...
    val metadataFactory =
      new ScalaInvocationMetadataFactory(Some(routes))
    val modulesConfig: ModulesConfig =
      new ScalaUserModulesConfig(Some(metadataFactory))
    val modulesConfigs: java.util.List[ModulesConfig] =
      List(modulesConfig).asJava
    Servers.create(
      serverUri,
      properties
