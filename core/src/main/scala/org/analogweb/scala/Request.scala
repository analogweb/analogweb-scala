...
case class Request(
  ...
  def query(name: String, defaultValue: String = ""): String =
    queryOption(name)
      .getOrElse(defaultValue)

  def queryOption(name: String): Option[String] =
    queries(name).headOption

  def queries(name: String): Seq[String] =
  ...
  def requestPath: RequestPathMetadata =
    context
      .getRequestPath()
}
