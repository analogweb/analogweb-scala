...
class CirceJsonValueResolver extends ScalaRequestValueResolver {
  ...
  def circeResolverContext2Decoder[A](context: ResolverContext): Option[Decoder[A]] = context match {
  ...
  override def resolve[T](
    ...
    }.left.map { parsingError => NoValuesResolved(key, this, requiredType) }
  }
